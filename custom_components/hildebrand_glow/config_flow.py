        ...
        if user_input is not None:
            ...
            try:
                ...
                if resources:
                    ...
                    self._ve_name = next((ve.get("name", "Smart Meter") for ve in self._virtual_entities if ve.get("veId") == selected_ve_id), "Smart Meter")
                    return await self.async_step_tariff()
                errors["base"] = "no_resources"
            except GlowmarktApiError:
                errors["base"] = "cannot_connect"
        options = [{"value": ve["veId"], "label": ve.get("name", "Unknown Location")} for ve in self._virtual_entities if ve.get("veId")]
        return self.async_show_form(step_id="select_entity", data_schema=vol.Schema({vol.Required(CONF_VIRTUAL_ENTITY): SelectSelector(SelectSelectorConfig(options=options, mode=SelectSelectorMode.LIST))}), errors=errors)
