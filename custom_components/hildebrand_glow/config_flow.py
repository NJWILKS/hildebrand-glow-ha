...
class HildebrandGlowConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_select_entity(
        ...
        if user_input is not None:
            ...
            try:
                ...
                if resources:
                    ...
                    self._ve_name = next(
                        (
                            ve.get("name", "Smart Meter")
                            for ve in self._virtual_entities
                            if ve.get("veId") == selected_ve_id
                        ),
                        "Smart Meter",
                    )
        ...
        options = [
            {
                "value": ve["veId"],
                "label": ve.get("name", "Unknown Location"),
            }
            for ve in self._virtual_entities
            if ve.get("veId")
        ]
    ...
    async def async_step_tariff(
        ...
        if user_input is not None:
            ...
            title = getattr(self, "_ve_name", None) or (
                f"Smart Meter ({self._user_data[CONF_USERNAME]})"
            )
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Required(
                        CONF_ELECTRICITY_STANDING_CHARGE,
                        default=DEFAULT_ELECTRICITY_STANDING_CHARGE,
                    ): vol.Coerce(float),
                    vol.Required(
                        CONF_GAS_RATE,
                        default=DEFAULT_GAS_RATE,
                    ): vol.Coerce(float),
                    vol.Required(
                        CONF_GAS_STANDING_CHARGE,
                        default=DEFAULT_GAS_STANDING_CHARGE,
                    ): vol.Coerce(float),
                }
            ),
        )
