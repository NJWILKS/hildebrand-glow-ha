...
from .ledger_coordinator import LedgerOnlyGlowmarktDataUpdateCoordinator
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # 2.5 deliberately has one historical population path.  The ledger worker is
    # config-entry-owned and non-blocking; no Recorder migration, cleanup, legacy
    # backfill or cost-statistics worker is started during this validation phase.
...
async def async_update_options(hass: HomeAssistant, entry: ConfigEntry) -> None:
    coordinator: LedgerOnlyGlowmarktDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    coordinator: LedgerOnlyGlowmarktDataUpdateCoordinator | None = hass.data.get(
        DOMAIN, {}
    ).get(entry.entry_id)
