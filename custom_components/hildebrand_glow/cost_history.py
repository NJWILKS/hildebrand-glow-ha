...
async def _backfill_cost_statistics(
    ...
    if state.get("_backfilled"):
        return

    imported: dict[str, int] = {}
    for commodity, (cost_classifier, usage_key, standing_key) in COMPONENT_KEYS.items():
    ...
    if imported:
        _LOGGER.info(
            "Backfilled Glowmarkt cost components: %s",
...
async def _refresh_tariff_ledger(
    ...
    for commodity, (cost_classifier, _, _) in COMPONENT_KEYS.items():
        resource = coordinator.resources.get(cost_classifier)
        if not resource:
            continue
...
async def async_cost_history_worker(
    ...
    """Backfill stackable cost history, then keep the tariff ledger current."""
